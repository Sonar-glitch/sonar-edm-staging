...
async function findAudioUrlEnhanced(artistName, trackName, spotifyPreviewUrl = null) {
  console.log(`🔍 Enhanced audio search for: ${artistName} - ${trackName}`);
  
  // TIER 1: Use Spotify preview if available
  if (spotifyPreviewUrl) {
    console.log(`✅ Using Spotify preview URL`);
    return {
  ...
  // TIER 2: Try Apple Music (existing function)
  console.log(`🔍 Trying Apple Music...`);
  const appleUrl = await findApplePreviewUrl(artistName, trackName);
  if (appleUrl) {
    return {
      audioUrl: appleUrl,
      source: 'apple',
  ...
  // TIER 3: Try SoundCloud
  console.log(`🔍 Trying SoundCloud...`);
  const soundcloudResult = await findSoundCloudAudioUrl(artistName, trackName);
  if (soundcloudResult) {
    return {
      audioUrl: soundcloudResult.audioUrl,
      source: 'soundcloud',
      duration: soundcloudResult.duration,
      quality: 'medium',
      metadata: soundcloudResult
    };
  }
  
  // TIER 4: Try YouTube (requires audio extraction)
  console.log(`🔍 Trying YouTube...`);
  const youtubeResult = await findYouTubeAudioUrl(artistName, trackName);
  if (youtubeResult) {
    // Note: In production, extract audio from YouTube video
    return {
      videoUrl: youtubeResult.videoUrl,
      source: 'youtube',
      quality: 'variable',
      metadata: youtubeResult,
      requiresExtraction: true
    };
  }
  
  // TIER 5: Try Beatport (for EDM)
  console.log(`🔍 Trying Beatport...`);
  const beatportResult = await findBeatportPreviewUrl(artistName, trackName);
  if (beatportResult) {
    return {
      audioUrl: beatportResult.audioUrl,
      source: 'beatport',
  ...
  console.log(`❌ No audio sources found for: ${artistName} - ${trackName}`);
  return null;
}
