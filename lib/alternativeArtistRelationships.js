...
class AlternativeArtistRelationships {
  ...
  /**
   * MISSING METHOD: Get artist similarity score between two artists
   * This is what the enhanced recommendation system is calling
   */
  async getArtistSimilarity(artist1, artist2) {
    if (!artist1 || !artist2) return 0;

    const normalizedArtist1 = this.normalizeArtistName(artist1);
    const normalizedArtist2 = this.normalizeArtistName(artist2);

    // Check if they're the same artist
    if (normalizedArtist1 === normalizedArtist2) {
      return 1.0; // Perfect match
    }

    // Check cache first
    const cacheKey = `similarity_${normalizedArtist1}_${normalizedArtist2}`;
    if (this.cache.has(cacheKey)) {
      return this.cache.get(cacheKey);
    }
    ...
    try {
      // Get similar artists for artist1
      const similarArtists = await this.getSimilarArtists(artist1, 20);
      
      // Check if artist2 is in the similar artists list
      const match = similarArtists.find(similar => 
        this.normalizeArtistName(similar.name) === normalizedArtist2
      );

      if (match) {
        similarity = match.similarity;
      } else {
        // Try reverse lookup - get similar artists for artist2
        const reverseSimilarArtists = await this.getSimilarArtists(artist2, 20);
        const reverseMatch = reverseSimilarArtists.find(similar => 
          this.normalizeArtistName(similar.name) === normalizedArtist1
        );
      ...
      console.log(`🎵 Artist similarity: ${artist1} <-> ${artist2} = ${(similarity * 100).toFixed(1)}%`);
      return similarity;

    } catch (error) {
      console.error(`❌ Error calculating artist similarity for ${artist1} <-> ${artist2}:`, error);
      return 0;
    }
  }
