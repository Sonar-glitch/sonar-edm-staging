...
const { EnhancedAudioAnalysisService } = require('./enhancedAudioAnalysisService');
...
this.enhancedAudioAnalysis = new EnhancedAudioAnalysisService();
    ...
    Object.entries(featureWeights).forEach(([feature, weight]) => {
      if (eventSound[feature] !== undefined && userProfile[feature] !== undefined) {
        ...
        if (feature === 'tempo') {
          ...
          const difference = Math.abs(eventSound[feature] - userProfile[feature]);
          similarity = Math.max(0, Math.exp(-difference * 3));
