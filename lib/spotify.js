// lib/spotify.js - Update with better error handling and removal of deprecated function

async function getTopArtists(token, timeRange = 'medium_term', limit = 20) { // Increased limit for better analysis
  try {
...
async function getTopTracks(token, timeRange = 'medium_term', limit = 50) { // Increased limit for better analysis
  try {
...
// async function getAudioFeaturesForTracks(token, trackIds) { ... }
...
module.exports = {
  getTopArtists,
  getTopTracks,
  // getAudioFeaturesForTracks, // EXPORT REMOVED
  refreshAccessToken
};
