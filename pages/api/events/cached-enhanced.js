...
// Avoid path alias to prevent prod resolution issues
...
async function triggerBackgroundRefresh(userId, db) {
  ...
  try {
    ...
    const base = process.env.NEXTAUTH_URL || process.env.VERCEL_URL || process.env.SITE_URL;
    if (!base) {
      console.warn('⚠️ No base URL set for background refresh; skipping fetch.');
      return;
    }
    const url = `${base.replace(/\/$/, '')}/api/events/enhanced?limit=20&background=true`;
