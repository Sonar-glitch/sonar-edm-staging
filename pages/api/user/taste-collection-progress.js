...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error('🔍 [Taste Progress API] Mongo connection error:', connErr.message);
      return res.status(503).json({
        success: false,
        error: 'DB_CONNECT_FAILED',
        message: connErr.message,
        status: { overall: 'error', detail: 'mongo_connection_failed' }
      });
    }
    ...
    console.error('🔍 [Taste Progress API] Error (unhandled):', { message: error.message, stack: error.stack });
